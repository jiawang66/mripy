...
class Device(object):
    ...
    def __init__(self, id_or_device):
        ...
        if isinstance(id_or_device, int):
            ...
            raise ValueError(f'Accepts int, Device or cupy.cuda.Device, got {id_or_device}')
...
def to_device(input, device=cpu_device):
    ...
    if odevice == cpu_device:
        with idevice: # why with idevice not odevice?
            return input.get()
    else:
        with odevice:
...
class Communicator(object):
    ...
    def _get_nccl_dtype_size(self, input):
        if input.dtype == np.float32:
            ...
            raise ValueError(f'dtype not supported, got {input.dtype}.')
